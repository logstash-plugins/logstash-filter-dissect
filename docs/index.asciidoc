...
There is a hybrid case where Dissect can be used to de-structure the section of the line that is reliably repeated and then Grok can be used on the remaining field fieldValues with # more regex predictability and less overall work to do. +
...
e.g. for a text of `1 2 3 go`, this `%{a} %{b} %{+a}` will build two key/fieldValues of `a => 1 3 go, b => 2` +
...
A later dissection can be done on fieldValues from a previous dissection or they can be independent.
...
Append fieldValues to the `tags` field when dissection fails
