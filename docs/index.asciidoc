...
*Behaviour when multiple delimiter are found in the source text:* +

[IMPORTANT]
===============================
Starting from version 1.1.1 of this plugin, multiple found delimiter handling has changed.
Now multiple delimters will be seen as missing fields by default and not padding.
...
....
John Smith,Big Oaks,Wood Lane,Hambledown,Canterbury,CB34RT
....
...
%{id} %{function->} %{server}
....
Note, above, the second field has a `->` suffix, this tells Dissect to ignore padding to its right. +
